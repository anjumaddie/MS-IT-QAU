...
# Ask the user for the number of vehicles to be included in the simulation
max_num_vehicles = int(input("Enter the number of vehicles to be included in the simulation (100, 200, or 300): "))
...
print(f"Cell size used: {cell_size} (Which is defined in 'cell_size.txt')")
        ...
        try:
            ...
            if vehicle_id in seen_vehicles:
                continue  # Skip if vehicle data is already collected
                        ...
                        # Stop collecting data after max_num_vehicles vehicles have been seen
            if len(seen_vehicles) >= max_num_vehicles:
                break
...
print(f"Total number of cells created: {len(cells)}")
